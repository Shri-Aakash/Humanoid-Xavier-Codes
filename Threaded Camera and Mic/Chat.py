...
class ChatBot():
    ...
    def Chat(self,sentence,frame,fr):
            ...
            if self.tag=='places':
                for intent in ChatBot.intents['intents']:
                    if self.tag == intent["tag"]:
                        return [random.choice(intent['responses']),'Location']
                ...
                if len(faces):
                    ...
                    if name=='Un-identified Face':
                        if self.tag == "follow me":
                            return ["Sorry your face is not in the database and thus i can't follow you.",'-']
                        else:
                            return ["Sorry this feature is only for faculty.",'-']
                    else:
                        for intent in ChatBot.intents['intents']:
                            if self.tag == intent['tag']:
                                return [f"{name} show me your QR",'QR']
                else:
                    #print("Your Face is not visible.Please Keep your face within the frame")
                    return ['Your Face is not visible.Please Keep your face within the frame','Face not visible']
            else:
                for intent in ChatBot.intents['intents']:
                    if self.tag == intent["tag"]:
                        return [random.choice(intent['responses']),'']
        else:
            return ['I do not understand','NaN']
        return ()
